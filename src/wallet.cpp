...
void CWallet::ResendWalletTransactions(bool fForce)
{
    ...
    CTxDB txdb("r");
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (fForce || nTimeBestReceived - (int64_t)wtx.nTimeReceived > 2.5 * 60)
...
// Electra: total coins staked (non-spendable until maturity)
...
// Electra: get current stake weight
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CElectraAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CElectraAddress(address).ToString());
}
...
// Electra: check 'spent' consistency between wallet and txindex
// Electra: fix wallet spent state according to txindex
...
// Electra: disable transaction (only for coinstake)
